...
def get_adjacency_matrix(distance_df_filename, num_of_vertices, id_filename=None):
    ...
    用于生成图的邻居矩阵和距离矩阵
    input:
        distance_df_filename: str, path of the csv file contains edges information      # 包括边信息文件路径
        num_of_vertices: int, the number of vertices                                    # 图中节点的数量

    return:
        A: np.ndarray                       # 邻接矩阵，表示节点间的连接关系
        adjacency matrix                    # 距离矩阵，表示节点间的关系
    """
...
def load_graphdata_channel1(
    ...
    """
    用于加载和处理图数据，构造适合时空序列预测的模型的 input

    input:
        graph_signal_matrix_filename    # 图信息矩阵文件名
        num_of_hours                    # 用于构建输入数据的时间窗口
        num_of_days
        num_of_weeks
        DEVICE                          # 设备
        batch_size                      # 批次大小
    ...
    return:                             # 返回训练，验证和测试数据加载器，以及数据的均值和标准差
        train_loader
        train_target_tensor
        val_loader
        val_target_tensor
    ...
    train_x = file_data["train_x"]  # train_x.shape: (7769, 307, 3, 72)

    # 批次，节点数，步长，特征维度
    # 取特征信息中第一个维度值（这里都是取特征信息中第一个维度，为什么？因为第一个特征信息比较重要或敏锐）
    train_x = train_x[:, :, 0:1, :]  # 取维度中第一个维度
    train_target = file_data["train_target"]  # (7769, 307, 12)
    ...
    )  # 批次，节点数（传感器数量），时间内采集数，特征维度  train_x_tensor.shape: [7769, 307, 1, 72]
    train_target_tensor = (
        torch.from_numpy(train_target).type(torch.FloatTensor).to(DEVICE)
    )  # (B, N, T)

    """
    用于包装数据，将输入和目标张量组合成一个数据集对象，方便通过索引访问
    """
    train_dataset = torch.utils.data.TensorDataset(train_x_tensor, train_target_tensor)

    # 用于批处理（将数据集划分成批次），迭代支持（在训练循环中遍历数据），数据打乱（减小模型对数据顺利的依赖）
    ...
    test_target_tensor = (
        torch.from_numpy(test_target).type(torch.FloatTensor).to(DEVICE)
    )
    ...
    # print
    print(
        "train:", train_x_tensor.size(), train_target_tensor.size()
    )  # train: [10181, 307, 1, 12] target: [10181, 307, 12]
    ...
    with torch.no_grad():
        ...
        for i in range(prediction_length):
            ...
            if metric_method == "mask":
                mae = masked_mae_test(
                    data_target_tensor[:, :, i], prediction[:, :, i], 0.0
                )
                ...
                rmse = (
                    mean_squared_error(data_target_tensor[:, :, i], prediction[:, :, i])
                    ** 0.5
                )
                mape = masked_mape_np(
                    data_target_tensor[:, :, i], prediction[:, :, i], 0
        ...
        if metric_method == "mask":
            mae = masked_mae_test(
                data_target_tensor.reshape(-1, 1), prediction.reshape(-1, 1), 0.0
            )
            rmse = masked_rmse_test(
                data_target_tensor.reshape(-1, 1), prediction.reshape(-1, 1), 0.0
            )
