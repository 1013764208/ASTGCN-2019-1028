...
def train_main():
    ...
    predict_main(
        best_epoch, test_loader, test_target_tensor, metric_method, _mean, _std, "test"
    )


def predict_main(
    global_step, data_loader, data_target_tensor, metric_method, _mean, _std, type
):
    """
...
if __name__ == "__main__":
    ...
    # 构建加载数据，返回训练，验证，测试数据加载器
    (
        train_loader,                           # 训练数据加载集（批次的）
        train_target_tensor,                
        val_loader,                             # 测试数据加载集
        val_target_tensor,
        test_loader,
    ...
    # 生成图的邻接矩阵和距离矩阵
    # adj_mx.shape: (307, 307)  distance_mx.shape:(307, 307)
